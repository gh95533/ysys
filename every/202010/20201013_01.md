...
​	~~还有一点就是gpssh,gpscp,gpssh-exkeys与greenplum的运行没有直接关系，或者可以将gpssh-exkyes，gpssh,gpscp就是简便工具帮助来进行文件拷贝文件修改文件上传文件打包文件下载文件解压的功能。也就是说，如果实在是各种因素下，可以每台服务器安装一下文件再进行部署。（这段是在20210529删除，之前在部署gp5.x时就是没有端口没有22，执行时会报错，故而删除此处论断）~~
