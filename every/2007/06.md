...
# oracle rac revise private ipaddress
...
​	最近需要学习rman duplicate,可是这需要两个数据库集群，需要搭建一套新的数据库集群，想到可以用虚拟机复制，得到一个数据库，不过需要修改一下数据库的地址，public,private,vip,scan都需要修改，在之前的学习中，已经完成了对public,vip,scan的处理，今天这个文档主要对于private来处理文档。(后来发现好像之前也是参考文档完成了对private的处理)
