...
# greenplum not free disk space and gprecoverseg
...
​	上面这个办法用的有的巧劲了，记得要对数据库进行备份，防止崩溃了,之后还要做到定时监控服务器空间问题，否则就坑了。

​	除了上面这个风险较高的方法，还有一个好的方案是故意在数据节点1和数据节点2上放置100g左右的文件强烈建议这件事情，数据库一天当中很少出现这么高的情况，建议进行上述操作。
